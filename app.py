...
BUTTON_PINS = [5, 13, 26, 24]
LED_PINS = [6, 19, 20, 21]

DATA_FILE = 'xlr_config.json'
PIN_CODE = "5511"

# FIX 1: Use a fixed key so restarts don't log you out
SECRET_KEY = "kh_xlr_switcher_secret_key_fixed"
...
class RelayController:
    def __init__(self, relay_pins, button_pins, led_pins):
        ...
        self.channels = []
        self.last_button_press = 0  # For software debounce
    ...
    def handle_physical_button(self, channel):
        ...
        # FIX 2: Safety Try/Except to prevent app crash
        try:
            # FIX 3: Extra Software Debounce
            current_time = time.time()
            if current_time - self.last_button_press < 0.2: # Ignore if pressed within 200ms
                return
    ...
    def set_all(self, state):
        for i, ch in enumerate(self.channels):
            ch['active'] = state
            if HARDWARE_PRESENT:
                GPIO.output(ch['pin'], GPIO.HIGH if state else GPIO.LOW)
...
# FIX 4: Ensure GPIO is cleaned up if the app crashes/exits
